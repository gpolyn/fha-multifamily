...
class Sec223fRefinance::Sec223fRefinance
  ...
  def initialize(args)
    ...
    if args[:lease]
      ...
      self.lease = Sec223fRefinance::Lease.new(args[:lease])
    ...
    if args[:tax_abatement]
      ...
      if args[:tax_abatement].each_key.any? {|k| k.to_s=~/^variable_phase/}
        self.tax_abatement = Sec223fRefinance::VariableAbatement.new(args[:tax_abatement])
        
        if tax_abatement.valid?
          args[:criterion_5][:tax_abatement_present_value] = tax_abatement.present_value
        end
      else
        self.tax_abatement = Sec223fRefinance::FixedAbatement.new(args[:tax_abatement])
    ...
    if args[:criterion_10].has_key?(:fha_inspection_fee) && !args[:criterion_10][:fha_inspection_fee].blank?
      crit_10_attrs = args[:criterion_10]
    else
      total_apartments = get_total_apartments args[:criterion_4]
      crit_10_attrs = criterion_10_attribute_handler(args[:criterion_10], total_apartments)
    end
    self.criterion_10 = Criterion10::Criterion10.new crit_10_attrs
    self.transaction_cost_criterion = criterion_10
    
    if args[:criterion_11]
      self.criterion_11 = Sec223fRefinance::Criterion11.new args[:criterion_11]
  ...
  def transaction_amount_including_loan_closing_charges
    transaction_cost_criterion.existing_indebtedness.to_f + total_development_costs - estimate_of_repair_cost.to_f
  end
  
  def estimate_of_repair_cost
    transaction_cost_criterion.repairs
  end
  ...
  def valid?
    ...
    if criterion_10
      criterion_10_is_valid = criterion_10.valid? 
      criterion_10.errors.each {|k,v| errors.add k, v } unless criterion_10_is_valid
    end
    ...
    val && criterion_10_is_valid && criterion_11_is_valid && lease_is_valid && tax_abatement_is_valid
  end
  ...
  def as_json(options={})
    criteria = super
    criteria[:criterion_10] = criterion_10.as_json
    criteria[:criterion_11] = criterion_11.as_json if criterion_11
    mtg_candidates = [criteria[:criterion_10][:line_i][:greater_of_line_g_or_line_h]]
  ...
  def criterion_10_attribute_handler(attrs, total_apts)
    ret_args = attrs.clone
    inspection_params = {:repairs => ret_args[:repairs], 
                         :total_apartments => total_apts}
    ret_args[:fha_inspection_fee] = Criterion10.inspection_fee inspection_params
    ret_args
  end
end
